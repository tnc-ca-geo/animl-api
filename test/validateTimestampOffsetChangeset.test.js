...
tape('validateTimestampOffsetChangeset - passes when project not found', async (t) => {
  try {
    Sinon.restore();
    MockConfig(t);

    Sinon.stub(ProjectSchema, 'findById').callsFake(() => null);

    // Should not throw
    await validateTimestampOffsetChangeset('nonexistent', { cameras: ['camera1'] });
    t.pass('Should not throw when project not found');
  } catch (err) {
    t.error(err);
  }
