...
import { BaseAuthedModel, Context, MethodParams, idMatch, roleCheck } from './utils.js';
import { ProjectModel } from './Project.js';

export class CameraModel {
  static async getWirelessCameras(input: { _ids?: string[] }, context: Context) {
  ...
  static async createWirelessCamera(
    input: { projectId?: string; cameraId: string; make?: string; model?: string },
    context: Context,
  ) {
    console.log('CameraModel.createWirelessCamera - input: ', input);
    const successfulOps = [];

    const { projectId = 'default_project', cameraId, make, model } = input;
