...
import { ImageErrorTaskConfig } from '../../../task/image-errors.js';
...
export class ImageErrorModel {
  ...
  static async clearErrors(input: { batch: string }) {
    try {
      await retry(
        async () => {
          return await ImageError.deleteMany({
            batch: input.batch,
          });
        },
        { retries: 2 },
   ...
   */
  static async exportErrorsTask(input: ImageErrorTaskConfig, context: Context) {
    try {
