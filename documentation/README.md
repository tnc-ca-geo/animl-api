...
Typically, the latter is used for the processing of images from non-wireless, SD-card based camera traps, from which users will collect large numbers of images–often tens/hundreds of thousands over the course of a deployment-and upload them all at once for inference.

Wireless cameras, on the other hand, produce steady streams of small numbers of images as they get triggered in the field, and Animl processes them immediately to support real-time inference and alerts. For more information on the different types of wireless camera traps, how they can be integrated with Animl, and use-cases they are well suited to support, see our [Wireless Camera Trapping Guide](https://guides.animl.camera/).
...
### Wireless camera trap data

#### Ingestion

Animl supports two different [types of wireless cameras](https://guides.animl.camera/fundamentals/types-of-wireless-camera-traps): cellular and locally-networked, mesh camera traps.
...
Locally-networked, mesh camera traps (like [Buckeye X80s](https://guides.animl.camera/tnc-wireless-camera-trap-documentation/buckeye-x80-networked-wireless-cameras)) can also take photos and transmit them in real-time, but rather than uploading them directly to the cloud via a cellular modem built into the camera, they transmit their images via VHF radio to an nearby base station (1), which is an internet-connected field computer running the **animl-base** app. **animml-base** watches for new images written to the disk, and when a new image file arrives it uploads it to the **animl-images-ingestion** Bucket (6) entrypoint. Mesh cameras are useful in locations that may have one point of internet connectivity (e.g. a Starlink connection at a field station) but the locations where you want to place cameras lack cell service.
...
### Bulk-uploaded camera trap data
...
- **animl-frontend** - _S3 Bucket_
  ...
  - Instructions on how to deploy animl-frontend [here](DeployFrontend.md).
